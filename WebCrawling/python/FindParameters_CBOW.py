...
for s in range (100, 500, 50):
    for w in range(3, 20):
        ...
        s6 = '휴식'
        s7 = '운동'
        dist1 = distance.euclidean(store_model.wv.word_vec(s1),(store_model.wv.word_vec(s2)))
        dist2 = distance.euclidean(store_model.wv.word_vec(s3),(store_model.wv.word_vec(s4)))
        dist3 = distance.euclidean(store_model.wv.word_vec(s5),(store_model.wv.word_vec(s6)))
        print ("Cosine similarity between " ,s1, s2, ":", store_model.similarity(s1, s2))
        print("Euclidean distance: ", dist1)
        print ("Cosine similarity between " ,s3, s4, ":", store_model.similarity(s3, s4))
        print("Euclidean distance: ", dist2)
        print ("Cosine similarity between " ,s5, s6, ":", store_model.similarity(s5, s6))
        print("Euclidean distance: ", dist3)
        
        print(s7, "단어와 유사도 높은 순서: ")
        print(store_model.wv.most_similar(positive=s7))
        X = store_model[vocab]
